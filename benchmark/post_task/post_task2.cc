...
private:
    ...
    {
        pool_.GetNextLoopWithHash(thread_index)
            ->RunInLoop([this, thread_index]() mutable {
...
int main(int argc, char* argv[])
{
    ...
    p.Start();
    p.Wait();
    LOG_WARN << argv[0] << " thread_count=" << thread_count
             << " post_count=" << post_count << " use time: " << p.use_time()
             << " seconds\n";
    return 0;
}
