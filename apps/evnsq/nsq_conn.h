...
public:
    enum Status
    {
        ...
        kIdentifying    = 2,
        kAuthenticating = 3,
        kConnected      = 4,  // After identifying and authenticating, we
                         // successfully connected to NSQD
        kSubscribing = 5,
        kReady
        = 6,  // Ready to produce messages to NSQD or consume messages from NSQD
        kDisconnecting = 7,
    };

    typedef std::function<void(const std::shared_ptr<NSQConn>& conn)>
        ConnectionCallback;
    typedef std::function<void(const CommandPtr& cmd, bool successfull)>
        PublishResponseCallback;
...
private:
    ...
    void Identify();
    void OnConnectedOK();  // After we do Identify/Authenticate successfully,
                           // this function will be called
    void OnConnectedFailed();  // After we do Identify/Authenticate failed, this
...
private:
    ...
    std::list<CommandPtr>
        wait_ack_;  // FIXME XXX TODO FIX std::list::size() performance problem
