...
struct GetResult
{
    GetResult()
        : code(NOT_FIND_RET)
    { }
    GetResult(const GetResult& result)
        : code(result.code)
        , value(result.value)
    { }
    ...
    GetResult(int c, const std::string& v)
        : code(c)
        , value(v)
    { }
    int code;
...
struct PrefixGetResult
{
    ...
    PrefixGetResult(PrefixGetResult&& result)
        : code(result.code)
...
typedef std::function<void(const std::string& key, const GetResult& result)>
    GetCallback;
...
typedef std::function<void(const std::string& key,
    const PrefixGetResultPtr result)>
    PrefixGetCallback;
typedef std::function<void(const PrefixMultiGetResult& result)>
    PrefixMultiGetCallback;
