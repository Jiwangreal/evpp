...
public:
    ...
    //      2. memcached集群模式，传输的参数可以是vbucket conf url ：
    //      "http://host:port/vbucket_conf"
    //      3. memcached集群模式，传输的参数可以是vbucket conf 本地文件：
    //      "/the/path/to/vbucket_conf"
    // @param[in] thread_num -
    // @param[in] timeout_ms -
    // @return  -
    MemcacheClientPool(const char* vbucket_conf,
        int thread_num,
        int timeout_ms,
    ...
    void Set(evpp::EventLoop* caller_loop,
    ...
    void Remove(evpp::EventLoop* caller_loop,
        const std::string& key,
        RemoveCallback callback);
    void Get(evpp::EventLoop* caller_loop,
        const std::string& key,
        GetCallback callback);
    void PrefixGet(evpp::EventLoop* caller_loop,
        const std::string& key,
        PrefixGetCallback callback);

    void MultiGet(evpp::EventLoop* caller_loop,
        std::vector<std::string>& keys,
        MultiGetCallback& callback);
    void RunBackGround(const std::function<void(void)>& fun);
    //  void MultiGetImpl(evpp::EventLoop* caller_loop,
    //  std::vector<std::string>& keys, MultiGetCallback callback);

    void PrefixMultiGet(evpp::EventLoop* caller_loop,
        std::vector<std::string>& keys,
        PrefixMultiGetCallback callback);
