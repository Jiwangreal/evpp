...
{
public:
    MemcacheClient(evpp::EventLoop* evloop,
        evpp::TCPClient* tcp_client,
        MemcacheClientBase* mcpool = nullptr,
        const int timeout_ms       = 249)
        : id_seq_(0)
        , exec_loop_(evloop)
        , tcp_client_(tcp_client)
        , mc_pool_(mcpool)
        , timeout_(timeout_ms * 1000 * 1000)
        , codec_(nullptr)
    ...
    void OnResponseData(const evpp::TCPConnPtr& tcp_conn, evpp::Buffer* buf);
