...
public:
    ...
    { }
    virtual void OnMultiGetCommandOneResponse(int resp_code,
        std::string& key,
        std::string& value)
    { }
    virtual void OnMultiGetCommandDone(int resp_code,
        std::string& key,
        std::string& value)
    { }
...
private:
    ...
    uint32_t id_;  // 并非全局id，只是各个memc_client内部的序号;
                   // mget的多个命令共用一个id
    uint16_t vbucket_id_;
    std::vector<uint16_t>
        server_id_history_;  // 执行时从多个备选server中所选定的server
};
...
{
public:
    SetCommand(evpp::EventLoop* evloop,
        uint16_t vbucket,
    ...
    virtual void OnError(int err_code)
    {
        ...
        {
            loop->RunInLoop(
                std::bind(set_callback_, std::move(key_), err_code));
        }
        else
        {
    ...
    virtual void OnSetCommandDone(int resp_code)
    {
        ...
        {
            loop->RunInLoop(
                std::bind(set_callback_, std::move(key_), resp_code));
        }
        else
        {
...
{
public:
    GetCommand(evpp::EventLoop* evloop,
        uint16_t vbucket,
        const std::string& key,
    ...
    virtual void OnError(int err_code)
    {
        ...
        if (loop && !loop->IsInLoopThread())
        {
            caller_loop()->RunInLoop(std::bind(get_callback_,
                std::move(key_),
...
{
public:
    PrefixGetCommand(evpp::EventLoop* evloop,
        uint16_t vbucket,
        const std::string& key,
        PrefixGetCallback callback)
        : Command(evloop, vbucket)
        , key_(key)
        , mget_callback_(callback)
        , mget_result_(std::make_shared<PrefixGetResult>())
    { }
    ...
    virtual void OnError(int err_code)
    {
        ...
        if (loop && !loop->IsInLoopThread())
        {
            loop->RunInLoop(std::bind(
                mget_callback_, std::move(key_), std::move(mget_result_)));
        }
        else
        {
...
public:
    MultiKeyHandler(const Callback& callback, uint32_t finished_nums = 0)
        : finished_serverid_nums_(finished_nums)
        , serverid_size_(0)
        , mget_callback_(callback)
    { }
...
{
private:
    typedef std::shared_ptr<MultiKeyHandler<Result, Callback>>
        MultiKeyHandlerPtr;

public:
    MultiKeyMode(const MultiKeyHandlerPtr& handler)
        : multikey_handler_(handler)
        , retry_stage_(false)
    { }
...
typedef std::shared_ptr<MultiKeyHandler<MultiGetResult, MultiGetCallback>>
    MultiKeyGetHandlerPtr;

class MultiGetCommand
    : public Command
    , public MultiKeyMode<MultiGetResult, MultiGetCallback>
{
public:
    MultiGetCommand(evpp::EventLoop* evloop,
        const uint16_t vbucket,
        const MultiKeyGetHandlerPtr& handler)
        : Command(evloop, vbucket)
        , MultiKeyMode(handler)
    { }
    ...
    virtual void OnError(int err_code)
    {
        ...
        {
            caller_loop()->RunInLoop(std::bind(get_handler()->get_callback(),
                std::move(get_handler()->get_result())));
        }
    }
    static void PacketRequest(const std::vector<std::string>& keys,
...
typedef std::shared_ptr<
    MultiKeyHandler<PrefixMultiGetResult, PrefixMultiGetCallback>>
    PrefixMultiKeyGetHandlerPtr;

class PrefixMultiGetCommand
    : public Command
    , public MultiKeyMode<PrefixMultiGetResult, PrefixMultiGetCallback>
{
public:
    PrefixMultiGetCommand(evpp::EventLoop* evloop,
        uint16_t vbucket,
        const PrefixMultiKeyGetHandlerPtr& handler)
        : Command(evloop, vbucket)
        , MultiKeyMode(handler)
    { }
    ...
    virtual void OnError(int err_code)
    {
        ...
        if ((finish + 1) >= get_handler()->serverid_size())
        {
            caller_loop()->RunInLoop(std::bind(
                get_handler()->get_callback(), get_handler()->get_result()));
        }
    }
    static void PacketRequest(const std::vector<std::string>& keys,
        const std::vector<uint16_t>& vbuckets,
...
{
public:
    RemoveCommand(evpp::EventLoop* evloop,
        uint16_t vbucket,
        const std::string& key,
    ...
    virtual void OnError(int err_code)
    {
        ...
        {
            loop->RunInLoop(
                std::bind(remove_callback_, std::move(key_), err_code));
        }
        else
        {
    ...
    virtual void OnRemoveCommandDone(int resp_code)
    {
        ...
        {
            loop->RunInLoop(
                std::bind(remove_callback_, std::move(key_), resp_code));
        }
        else
        {
