...
void TestTCPClientConnectFailed()
{
    ...
    std::shared_ptr<evpp::TCPClient> client(
        new evpp::TCPClient(loop.get(), GetListenAddr(), "TCPPingPongClient"));
    client->SetConnectionCallback(
        [&loop, &client](const evpp::TCPConnPtr& conn) {
...
void TestTCPClientDisconnectImmediately()
{
    std::shared_ptr<evpp::EventLoop> loop(new evpp::EventLoop);
    std::shared_ptr<evpp::TCPClient> client(
        new evpp::TCPClient(loop.get(), "cmake.org:80", "TCPPingPongClient"));
...
struct NSQConn
{
    NSQConn(evpp::EventLoop* loop)
        : loop_(loop)
    {
        client_ = std::make_shared<evpp::TCPClient>(
            loop, "www.so.com:80", "TCPPingPongClient");
    ...
    void Disconnect()
    {
        if (!connected_)
        {
            loop_->RunAfter(100.0, [this]() {
                this->Disconnect();
            });
            return;
        }

        // We call TCPClient::Disconnect and then delete the hold object of
        // TCPClient immediately
        client_->Disconnect();
        client_.reset();
        connected_ = false;
        loop_->RunAfter(100.0, [this]() {
...
void TestTCPClientConnectLocalhost()
{
    ...
    client.SetMessageCallback(
        [](const evpp::TCPConnPtr& conn, evpp::Buffer* buf) {});
    client.Connect();
    loop.Run();
    assert(evpp::GetActiveEventCount() == 0);
}
