    ...
    const evpp::http::HTTPSendResponseCallback& cb)
{
    ...
    {
        cb("Format error. Please try again. The valid format is "
           "[http://127.0.0.1:9099/proxy?http://www.so.com/robots.txt]");
        return;
    }

    std::string proxy_url = uri.substr(index + 1);
    std::shared_ptr<evpp::httpc::GetRequest> r(
        new evpp::httpc::GetRequest(loop, proxy_url, evpp::Duration(3.0)));
    auto f
        = [cb, ctx, r](const std::shared_ptr<evpp::httpc::Response>& response) {
...
int main(int argc, char* argv[])
{
    ...
    server.SetThreadDispatchPolicy(
        evpp::ThreadDispatchPolicy::kIPAddressHashing);
