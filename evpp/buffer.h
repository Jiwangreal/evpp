...
public:
    ...
    // portion of the buffer. The data is valid for use only until the next
    // buffer modification (that is, only until the next call to a method like
    // Read, Write, Reset, or Truncate). The data aliases the buffer content at
...
private:
    ...
    void grow(size_t len)
    {
        ...
        else
        {
            ...
            memmove(begin() + reserved_prepend_size_,
                begin() + read_index_,
                length());
            read_index_  = reserved_prepend_size_;
