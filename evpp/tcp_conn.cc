    ...
    , status_(kDisconnected)
{
    ...
    DLOG_TRACE << "TCPConn::[" << name_ << "] channel=" << chan_.get()
               << " fd=" << sockfd << " addr=" << AddrToString();
}

TCPConn::~TCPConn()
{
    DLOG_TRACE << "name=" << name() << " channel=" << chan_.get()
...
void TCPConn::Close()
{
    ...
    // Use QueueInLoop to fix TCPClient::Close bug when the application delete
    // TCPClient in callback
    loop_->QueueInLoop(f);
}
...
void TCPConn::Send(const std::string& d)
{
    ...
    {
        loop_->RunInLoop(
            std::bind(&TCPConn::SendStringInLoop, shared_from_this(), d));
    }
}
...
void TCPConn::Send(const Slice& message)
{
    ...
    {
        loop_->RunInLoop(std::bind(&TCPConn::SendStringInLoop,
            shared_from_this(),
            message.ToString()));
    }
}
...
void TCPConn::SendInLoop(const void* data, size_t len)
{
    ...
    {
        LOG_INFO << "writing directly................";
        nwritten = ::send(
            chan_->fd(), static_cast<const char*>(data), len, MSG_NOSIGNAL);
        if (nwritten >= 0)
        {
            ...
            {
                loop_->QueueInLoop(
                    std::bind(write_complete_fn_, shared_from_this()));
            }
        }
        else
        {
            ...
            {
                LOG_ERROR << "SendInLoop write failed errno=" << serrno << " "
                          << strerror(serrno);
    ...
    if (remaining > 0)
    {
        ...
        if (old_len + remaining >= high_water_mark_
            && old_len < high_water_mark_ && high_water_mark_fn_)
        {
            loop_->QueueInLoop(std::bind(
                high_water_mark_fn_, shared_from_this(), old_len + remaining));
        }

        output_buffer_.Append(
            static_cast<const char*>(data) + nwritten, remaining);
...
void TCPConn::HandleRead()
{
    ...
    else if (n == 0)
    {
        if (type() == kOutgoing)
        {
            // This is an outgoing connection, we own it and it's done. so close
            // it
            DLOG_TRACE << "fd=" << fd_
                       << ". We read 0 bytes and close the socket.";
            status_ = kDisconnecting;
            HandleClose();
        }
        else
        {
            // Fix the half-closing problem :
            // https://github.com/chenshuo/muduo/pull/117
            ...
            else
            {
                ...
                DLOG_TRACE << "channel (fd=" << chan_->fd()
                           << ") DisableReadEvent. And set a timer to delay "
                              "close this TCPConn, delay time "
                           << close_delay_.Seconds() << "s";
                delay_close_timer_ = loop_->RunAfter(close_delay_,
                    std::bind(&TCPConn::DelayClose,
                        shared_from_this()));  // TODO leave it to user layer
                                               // close.
            }
        }
    }
...
void TCPConn::HandleWrite()
{
    ...
    ssize_t n = ::send(
        fd_, output_buffer_.data(), output_buffer_.length(), MSG_NOSIGNAL);
    if (n > 0)
    {
    ...
    else
    {
        ...
        {
            LOG_WARN << "this=" << this
                     << " TCPConn::HandleWrite errno=" << serrno << " "
                     << strerror(serrno);
        }
        else
        {
...
{
    assert(loop_->IsInLoopThread());
    DLOG_TRACE << "addr=" << AddrToString() << " fd=" << fd_
               << " status_=" << StatusToString();
    status_ = kDisconnecting;
...
void TCPConn::HandleClose()
{
    ...
    // But we call HandleClose() from out of TCPConn's method, the status_ is
    // kDisconnecting
...
void TCPConn::SetHighWaterMarkCallback(const HighWaterMarkCallback& cb,
    size_t mark)
{
