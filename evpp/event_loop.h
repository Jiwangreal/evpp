...
public:
    ...
    // so we can embed an EventLoop object into the old applications based on
    // libevent NOTE: Be careful to deal with the destructing work of
    // event_base_ and watcher_ objects.
...
private:
    ...
    std::mutex mutex_;
    // We use this to notify the thread when we put a task into the
    // pending_functors_ queue
    std::shared_ptr<PipeEventWatcher> watcher_;
    // When we put a task into the pending_functors_ queue,
    // we need to notify the thread to execute it. But we don't want to notify
    // repeatedly.
