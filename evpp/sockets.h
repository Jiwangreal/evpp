...
// @brief Parse a literal network address and return an internet protocol family
// address
// @param[in] address - A network address of the form "host:port" or
// "[host]:port"
// @return bool - false if parse failed.
EVPP_EXPORT bool ParseFromIPPort(const char* address,
...
EVPP_EXPORT bool SplitHostPort(const char* address,
    std::string& host,
    int& port);
...
inline struct sockaddr* sockaddr_cast(struct sockaddr_in* addr)
{
    return static_cast<struct sockaddr*>(
        evpp::sock::implicit_cast<void*>(addr));
}
...
{
    return static_cast<struct sockaddr_in*>(
        evpp::sock::implicit_cast<void*>(addr));
}
...
{
    return static_cast<struct sockaddr_in6*>(
        evpp::sock::implicit_cast<void*>(addr));
}

inline const struct sockaddr_in* sockaddr_in_cast(
...
inline const struct sockaddr_storage* sockaddr_storage_cast(
