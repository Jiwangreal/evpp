...
//      3. Call TCPClient::Connect() to try to establish a connection with
//      remote server
...
public:
    ...
    // @param[in] remote_addr - The remote server address with format
    // "host:port"
    //  If the host is not IP, it will automatically do the DNS resolving
    //  asynchronously
    // @param[in] name -
...
public:
    ...
    // establishes a connection or an exist connection breaks down or failed to
    // establish a connection. When these three events happened, the value of
    // the parameter in the callback is:
    //      1. Successfully establish a connection : TCPConn::IsConnected() ==
    //      true
    //      2. An exist connection broken down : TCPConn::IsDisconnecting() ==
    //      true
    //      3. Failed to establish a connection : TCPConn::IsDisconnected() ==
    //      true and TCPConn::fd() == -1
...
private:
    EventLoop* loop_;
    std::string local_addr_;  // If the local address is not empty, we will bind
                              // to this local address before doing connect()
