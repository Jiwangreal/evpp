...
EVPP_EXPORT evhttp_connection* evhttp_connection_base_new(
    ...
    //   libevent1.4 use '#define evtimer_set(ev, cb, arg)  event_set(ev, -1, 0,
    //   cb, arg)' to assign a timer, but '#define event_initialized(ev)
    //   ((ev)->ev_flags & EVLIST_INIT && (ev)->ev_fd !=
    //   (int)INVALID_HANDLE_VALUE)' So, if we use a event timer on windows,
    //   event_initialized(ev) will never return true.
