...
bool Service::initSSL(bool force_enable)
{
    ...
    SSL_CTX_set_options(ctx,
        SSL_OP_SINGLE_DH_USE | SSL_OP_SINGLE_ECDH_USE
            | SSL_OP_NO_SSLv2 /*禁用SSLv2*/ | SSL_OP_NO_TLSv1 /*禁用TLSv1*/);
    ...
    {
        LOG_ERROR << "Load private key file(" << private_key_file_.c_str()
                  << ")failed";
        ERR_print_errors_fp(stderr);
        return false;
    }
...
bool Service::Listen(int listen_port)
{
    ...
    evhttp_bound_socket_
        = evhttp_bind_socket_with_handle(evhttp_, "0.0.0.0", listen_port);
...
void Service::Pause()
{
    ...
    if (evhttp_bound_socket_)
    {
        evconnlistener_disable(
            evhttp_bound_socket_get_listener(evhttp_bound_socket_));
    }
#else
...
void Service::RegisterHandler(const std::string& uri,
...
void Service::HandleRequest(struct evhttp_request* req)
{
    ...
    {
        // This will forward to HTTPServer::Dispatch method to process this
        // request.
        auto f
            = std::bind(&Service::SendReply, this, ctx, std::placeholders::_1);
...
void Service::DefaultHandleRequest(const ContextPtr& ctx)
{
    ...
    {
        evhttp_send_reply(ctx->req(),
            HTTP_BADREQUEST,
            g_http_code_string[HTTP_BADREQUEST],
            nullptr);
    }
}
...
void Service::SendReply(const ContextPtr& ctx, const std::string& response_data)
{
    ...
    auto f = [this, response]() {
        ...
        if (!response->buffer)
        {
            evhttp_send_reply(x->req(),
                HTTP_NOTFOUND,
                g_http_code_string[HTTP_NOTFOUND],
                nullptr);
            return;
        }
    ...
    else
    {
        LOG_WARN
            << "this=" << this
            << " listening thread is going to stop. we discards this request.";
