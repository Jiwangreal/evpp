...
bool Server::Init(int listen_port)
{
    ...
    lt.thread->set_name(std::string("StandaloneHTTPServer-Main-")
    ...
    {
        int serrno = errno;
        LOG_ERROR << "this=" << this << " http server listen at port "
                  << listen_port << " failed. errno=" << serrno << " "
                  << strerror(serrno);
        lt.hservice->Stop();
        return false;
    }
...
bool Server::Init(const std::string& listen_ports /*"80,8080,443"*/)
{
    ...
    for (auto& s : vec)
    {
        ...
        if (i <= 0)
        {
            LOG_ERROR << "this=" << this << " Cannot convert [" << s
                      << "] to a integer. 'listen_ports' format wrong.";
            return false;
        }
        v.push_back(i);
    }
...
bool Server::Start()
{
    ...
    for (auto& lt : listen_threads_)
    {
        ...
        auto http_close_fn = [hservice, this]() {
            hservice->Stop();
            DLOG_TRACE << "http service at 0.0.0.0:" << hservice->port()
                       << " has stopped.";
        ...
        {
            auto cb = std::bind(
                &Server::Dispatch, this, _1, _2, _3, default_callback_);
...
void Server::Stop()
{
    ...
    for (auto& lt : listen_threads_)
    {
        ...
        auto fn                      = [&count, &promise, this, hs]() {
            ...
            if (count.fetch_add(1) + 1
                == static_cast<int>(listen_threads_.size()))
            {
                promise.set_value();
            }
        };
    ...
    for (auto& lt : listen_threads_)
    {
        // Service::Stop will be called automatically when listen_thread_ is
        // existing
        lt.thread->Stop(true);
    }
...
void Server::RegisterHandler(const std::string& uri,
    ...
    const HTTPRequestCallback& user_callback)
{
    ...
    DLOG_TRACE << "dispatch request " << ctx->req()
               << " url=" << ctx->original_uri()
               << " in main thread. status=" << StatusToString();
    if (!IsRunning())
    {
        LOG_WARN << "The listening thread is not running, may be it is "
                    "stopping now.";
