...
bool ParseFromIPPort(const char* address, struct sockaddr_storage& ss)
{
    ...
    else if (rc < 0)
    {
        ...
        {
            LOG_WARN << "ParseFromIPPort evutil_inet_pton (AF_INET, '"
                     << host.data() << "', ...) failed : " << strerror(serrno);
        }
        return false;
    }
...
std::string ToIPPort(const struct sockaddr_storage* ss)
{
    ...
    if (ss->ss_family == AF_INET)
    {
        ...
        const char* addr          = ::evutil_inet_ntop(
            ss->ss_family, &addr4->sin_addr, buf, INET_ADDRSTRLEN);
    ...
    else if (ss->ss_family == AF_INET6)
    {
        ...
        const char* addr           = ::evutil_inet_ntop(
            ss->ss_family, &addr6->sin6_addr, buf, INET6_ADDRSTRLEN);
