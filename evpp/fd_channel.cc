...
void FdChannel::AttachToLoop()
{
    ...
    assert(!attached_);
    ::event_set(
        event_, fd_, events_ | EV_PERSIST, &FdChannel::HandleEvent, this);
    ...
    {
        LOG_ERROR << "this=" << this << " fd=" << fd_ << " with event "
                  << EventsToString() << " attach to event loop failed";
    }
}
...
void FdChannel::DetachFromLoop()
{
    ...
    {
        LOG_ERROR << "DetachFromLoop this=" << this << "fd=" << fd_
                  << " with event " << EventsToString()
